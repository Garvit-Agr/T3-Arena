document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // 1. THE BOUNCER & NAME TAG LOGIC
    // ==========================================
    const loggedInUser = sessionStorage.getItem("arena_auth_user");
    
    if (!loggedInUser) {
        // If there is no ticket, kick them back to login immediately!
        console.warn("SECURITY BREACH: Unauthorized access attempted.");
        window.location.replace("login.html"); 
        return; 
    }

    // Grab the element we just added the ID to in HTML
    const nameDisplay = document.getElementById('operator-name-display');
    if (nameDisplay) {
        // Swap out 'OPERATOR_01' for their real name
        nameDisplay.innerText = loggedInUser.replace('_', ' ').toUpperCase();
    }
    ...
    // ==========================================
    // 3. BACKEND INTEGRATION: REAL DATA
    // ==========================================
    ...
    // Re-fetch the data every 5 seconds to keep the Lobby updated!
